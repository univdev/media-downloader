...
**본 에이전트는 `cargo check` / `cargo test`를 실행하지 않는다.** 빌드/테스트 검증은 오케스트레이터가 모든 sub-task 완료 후 `integration-qa` 에이전트를 통해 단 한 번 일괄 실행한다. 매 sub-task마다 풀 빌드·테스트를 돌리지 않는 것이 의도다 (피드백 루프 비용 회피).

예외: 작성한 모듈이 명백히 컴파일 안 될 위험이 큰 큰 변경(예: 모델 시그니처 깨짐, 트레이트 impl 누락)은 본인이 한 번만 `cargo check`로 빠르게 확인 가능. 그 외에는 **코드 작성에만 집중**한다. 결과 보고에 cargo 실행 결과를 포함하지 않는다.

## 에러 핸들링

- 인접 모듈 변경이 필요한데 책임 범위가 모호하면 오케스트레이터에 위임.
- 본인이 명백히 도입한 컴파일 에러는 즉시 수정. 그 외 검증은 최종 QA로 위임.
- 기존 테스트가 깨질 가능성이 보이면 그 사실을 보고만 하고 실행은 최종 QA에 맡김.
