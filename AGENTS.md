...
**팀 구성 (참조):** `rust-backend-engineer` + `frontend-engineer` + `test-engineer` + `integration-qa` (4명, opus). Codex 에이전트 정의는 `.codex/agents/`, 도메인 스킬은 `.agents/skills/` 참조. Claude 원본은 `.claude/agents/`, `.claude/skills/`에 보관되어 있다.
...
| 2026-05-05 | Codex 하네스 포인터 정리 | `.claude/agents` 원본을 Codex용 `.codex/agents/*.toml`로 읽고, `.claude/skills` 원본을 `.agents/skills`로 읽도록 AGENTS.md 경로 명시 | Codex 세션에서 하네스 에이전트/스킬을 자동 인식하도록 설정 |
